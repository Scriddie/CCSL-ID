...
TODOs
\begin{enumerate}
    \item save some jointplots from generate\_data and compare to sachs jointplots!
    \item There is data1 and data\_interv1!!!
    \item Am I using the mask wrong? Where are my bimodal marginals???
    \item Their 'perfect' interventions are still stochastic, right?
    \item Can they still infer directionality uner shift\_intervention with their but?
    \item Run on sachs data!
    \item Not quite sure what's going on with num\_regimes dimensions at base model creation
    \item How can they tell cause from effect under shift\_normal interventions that get rescaled to standard normal immediately?
    \item What does their interv\_mask really do?
    \item Go through 'forward\_given\_params' in base\_model
    \item How do they generate data?
    ...
    \item Are we really only looking at conditional probabilities?
    \item Can we play around with likelihoods? e.g. use uniform or something?
    \item Do we need full int coverage?
    ...
    \item How do we still explain the cause variable after the edge has gone? is it just the bias parameters??
    \item What exactly happened with the mask when the bug was there?
\end{enumerate}
...
\begin{enumerate}
    ...
    \item They are scaling each interventional distribution separately right after generation (dat\_generator.py). This might lose a lot of information? In their default settings they use uniform, so it is less severe. But for shift\_normal, it completely defeats the purpose of the intervention.
...
\begin{enumerate}
    ...
    \item We have to scae each regime individually for likelihood, but doesn't that also loose some (a lot of) information? is there a logical contradiction?
    \item Why doesn't it just work even without additive noise?
\end{enumerate}

\noindent
What is actually going on in their DCDI-G model?
\begin{enumerate}
    \item 
\end{enumerate}
