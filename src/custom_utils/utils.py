...
def snap(opt, fname=None):
    if not os.path.exists(opt.out_dir):
        os.mkdir(opt.out_dir)
    if fname is None:
        fig_path = os.path.join(opt.out_dir,
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    else:
        fig_path = os.path.join(opt.out_dir, fname)
    overwrite_folder(fig_path)
...
def thresholds(x):
    ...
    return {"lingamIC":               -np.inf,
            "lingamCV":               -x,
            "fges":                   -0.,  # favourable edge treatment
            "pc":                     -0.,  # favourable edge treatment
            "cam":                    -np.inf,
            "std_vsb_exploit":        -np.inf,
            "sortnregress":           x,
            "randomregress":          x,
            "randomregressIC":        -np.inf,
            "randomregressCV":        -np.inf,
            "sortnregress_p":         0.95,
            "sortnregressIC":         -np.inf,
            "sortnregressCV":         -np.inf,
            "notearsLinear":          x,
            "golemEV":                x,
            "golemNV":                x,
            "golemEV_golemNV":        x,
...
def loss_fn_template(loss_fn_name, data, w_est, edge_indices, params, *args, **kwargs):
    """ Optimize a loss function for a given structure. Make sure structure is DAG """
    d = data.shape[1]
    w_est = w_est.flatten()
    w_est[edge_indices] = params
    w_est = w_est.reshape(d, d)
    assert is_dag(w_est)
    return eval(loss_fn_name)(data, w_est, *args, **kwargs)
...
def notears_loss(data, w_est, lambda1):
    """ NoTears loss function """
    return (np.mean(np.square(data - np.dot(data, w_est)))
            + lambda1*np.sum(np.abs(w_est)))


def golem_ev_loss(data, w_est, lambda1):
    """ GOLEM loss function. Det and H will be zero for any dag """
    d = len(w_est)
    return (1/2*np.log(np.sum(np.square(data - np.dot(data, w_est))))
          - np.log(np.linalg.det(np.eye(d) - w_est))
          + lambda1*np.sum(np.abs(w_est))
        #   + acyclicity*_h(w_est)
          )


def golem_nv_loss(data, w_est, lambda1):
    """ GOLEM loss function. Det and H will be zero for any dag """
    d = len(w_est)
    return (1/2*np.sum(np.log(np.sum(np.square(data - np.dot(data, w_est)), axis=0)))
          - np.log(np.linalg.det(np.eye(d) - w_est))
          + lambda1*np.sum(np.abs(w_est))
    )


def bic_loss(data, w_est, lambda1):
    k = len(np.flatnonzero(w_est))
    n = data.shape[0]
    return n*golem_nv_loss(data, w_est, lambda1=0) + 1/2*k*np.log(n)  # maximise bic -> minimize -bic
# -------------------- END loss functions --------------------

def get_bins(n):
    return np.linspace(0, 1, num=n+1)[1:]
...
def varsortability(X, W, pairwise=False):
    E = W != 0
    Ek = E.copy()
...
def varsortability_old(X, W, pairwise):
    """
    ...
    # variance sorted adjacency matrix
    s = np.argsort(np.var(X, 0)).reshape(-1, 1)
    A = np.copy(W[s, s.T] != 0)
    P = np.copy(A)

    # entries above the diagonal: less variance ->-> more variance edge/path
    # entries below the diagonal: more variance ->-> less variance edge/path
    # entries on the diagonal: not a dag!
    n_paths = 0
    n_correctly_ordered_paths = 0
    if pairwise:
        maxpathlength = 1
    else:
        maxpathlength = P.shape[0] - 1
    for k in range(maxpathlength):
        n_paths += P.sum()
        n_correctly_ordered_paths += np.triu(P, 1).sum()
        P = P.dot(A)
    return n_correctly_ordered_paths / n_paths
